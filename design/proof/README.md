...
| `check_e1_e2.py` | Runs the real `python/hb_oracle.py` (and a copy with the proof's publish-then-tick order) over hand-made `hb_events` dumps of the E1 (rtraw, block 0 first) and E2 (two unordered `.STRONG` stores, barrier, weak load) cases; output recorded in its docstring. | Synthetic dumps, no GPU: evidence about the algorithm, not the corpus. Seed for T6's E1/E2 test files. |

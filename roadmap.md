...
| Exact unbounded vector clocks don't scale (O(threads) joins under heavy barriers; ~4× of the 8.5× on reduction) | 2, 6 | Exact only to match the oracle on the corpus; FastTrack epoch collapse + bounded clocks are the knobs; `YOSEMITE_HB_MODE=scalar-clock` isolates the cost for calibration |
